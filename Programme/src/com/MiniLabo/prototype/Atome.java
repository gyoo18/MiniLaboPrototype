...
public class Atome{
    ...
    public double mN;                   //Masse du noyeau
    ...
    * Est -1 s'il n'y a aucune liaison.*/
    ...
    public Atome(int nombreProton){
        ...
        NP = nombreProton;
        m = (double)NP*2.0*mP; //Calculer la masse de l'atome.
        mN = (double)NP*2.0*mP;
    ...
    */
    public static void ÉvaluerForces(Atome A){
        ...
        for (int iBoite = -1*Bfrontiere; iBoite < Bfrontiere + 1; iBoite++) {
            for (int jBoite =  -1*Bfrontiere; jBoite < Bfrontiere + 1; jBoite++) {
                for (int kBoite =  -1*Bfrontiere; kBoite < Bfrontiere + 1; kBoite++) {
                    boolean BoitePrincipale = iBoite==0 && jBoite==0 && kBoite==0;
                    ...
                    for (int i = 0; i < Environnement.size(); i++) {
                        ...
                        V3 PBoite= new V3 (0.0,0.0,0.0);
                        if (BoitePrincipale){
                            if(Environnement.get(i) == A /*  && iBoite==0 && jBoite==0 && kBoite==0  */){
                                //Si l'autre atome (A') est cet atome (A), sauter au prochain atome
                                continue;
                            }
                        ...
                        if( App.p.distForceÉval==0.0? true : dist<App.p.distForceÉval*(A.rayonCovalent+APrime.rayonCovalent)) {
                            ...
                            // Determine si A' est un voisin de A
                            ...
                            if (ListeForce[2]){
                                ...
                                if (!voisin){
                                    ...
                                    A.Force.addi( ForceÉlectrique(A.charge-2.0* (double)A.doublets, APrime.charge-2.0*(double)APrime.doublets,dist,dir)); //Appliquer la force électrique
                                }
...
//Il faut figurer ce qui se passe avec ListeForce
        for (int j = 0; j < A.forceDoublet.size(); j++) {
            for (int k = 0; k < A.forceDoublet.size(); k++) {
                ...
                if (ListeForce[2]){
                    A.forceDoublet.get(j).addi( ForceÉlectrique(-2.0, -2.0, eDist, eDir)); //Appliquer la force électrique entre les deux doublet   
                }
            }
        }
        ...
        //Appliquer les forces des doublets sur l'atome.
           for (int i = 0; i < A.positionDoublet.size(); i++) {
            ...
            if(A.forceDoublet.get(i).longueur() > 0){
                Sin0D = V3.croix(A.positionDoublet.get(i),A.forceDoublet.get(i)).longueur()/(A.positionDoublet.get(i).longueur()*A.forceDoublet.get(i).longueur());
                aTD = V3.norm( V3.croix( V3.croix(A.positionDoublet.get(i),A.forceDoublet.get(i)), A.positionDoublet.get(i)) );
            }else{
                Sin0D = 0.0;
                aTD = new Vecteur3D(0);
            }
            if(A.Force.longueur() > 0){
                Sin0N = V3.croix(A.positionDoublet.get(i),A.Force).longueur()/(A.positionDoublet.get(i).longueur()*A.Force.longueur());
                aTN = V3.norm( V3.croix( V3.croix(A.positionDoublet.get(i),A.Force), A.positionDoublet.get(i)) );
            }else{
                Sin0N = 0.0;
                aTN = new Vecteur3D(0);
            }
            
            //A.Force.addi(A.forceDoublet.get(i));
            //A.forceDoublet.get(i).addi(A.Force);
            A.Force =             V3.addi(V3.mult(V3.addi(A.Force, A.forceDoublet.get(i)),1.0),V3.mult(V3.addi(V3.mult(aTD.opposé(),A.forceDoublet.get(i).longueur()*(A.mN)*Sin0D),(V3.mult(aTN,         A.Force.longueur()*2.0*mE*Sin0N))),1.0));
            //A.forceDoublet.set(i, V3.addi(V3.mult(V3.addi(A.Force, A.forceDoublet.get(i)),1.0),V3.mult(V3.addi(V3.mult(aTD,         A.forceDoublet.get(i).longueur()*(A.mN)*Sin0D),(V3.mult(aTN.opposé(),A.Force.longueur()*2.0*mE*Sin0N))),1.0)));
        }
        A.Force = V3.mult(A.Force, A.mN/A.m);
        for (int i = 0; i < A.positionDoublet.size(); i++) {
    ...
    private static Vecteur3D ForceÉlectrique(double q1, double q2, double r, Vecteur3D dir){
        ...
        /* if ((force.longueur())>Math.pow(10,10)){
            System.err.println("Forc electrique forte");
        } */
    ...
    private static Vecteur3D ForcePaulie(double RayonCovalent1, double RayonCovalent2, double dist, Vecteur3D dir){
        Vecteur3D force = ( V3.mult(dir, Paramètres.ModulePaulie*(Math.pow(Paramètres.DistancePaulie*(RayonCovalent1+RayonCovalent2),13.0)/Math.pow(dist,13.0)) ));
        /* if ((force.longueur())>Math.pow(10,15)){
            System.err.println("Force forte Paulie");
            force = V3.mult(dir, Math.pow(10,15));
        } */
    ...
    private static Vecteur3D ForceDeMorse(double dist, Vecteur3D dir, int nLiaisons, int NP, int NPA){
        ...
        /* if ((force.longueur())>Math.pow(10,10)){
            System.err.println("Forc Morse forte");
        } */
        if (Double.isNaN(force.longueur())){
            App.p.répéter = true;
    ...
    private static Vecteur3D ForceTorsion(Vecteur3D IAxe, Vecteur3D JAxe, double mI, double mJ, int NBLiens, int NBDoublets, int K, int I, int J){
        //Compter le nombre de liens formés par A
        Vecteur3D PlanAjAi= V3.croix(JAxe,IAxe);
        
        Vecteur3D potdirection = V3.norm(V3.croix(IAxe,PlanAjAi));
        ...
        if(I == -1 || J == -1){
            ...
            Kij = Math.pow(fréquenceFondamentale,2.0)*masse*Paramètres.ModuleTorsion; //Force du ressort angulaire
        }
    ...
    private static Vecteur3D ForceTorsion(Atome Ai, Atome A, Atome Aj){
        ...
        return ForceTorsion(IAxe, JAxe, Ai.m, Aj.m, NBLiens, A.doublets, A.NP, Ai.NP, Aj.NP);
    }
    ...
    private static Vecteur3D ForceTorsion(Atome Ai, Atome A,  int Doubletj){
       ...
       return ForceTorsion(IAxe, A.positionDoublet.get(Doubletj), Ai.m, 2.0*mE, NBLiens, A.doublets, A.NP, Ai.NP, -1);
    }
    ...
    private static Vecteur3D ForceTorsion(int Doubleti, Atome A,  Atome Aj){
        ...
        return ForceTorsion(A.positionDoublet.get(Doubleti), JAxe, 2.0*mE, Aj.m, NBLiens, A.doublets, A.NP, -1, Aj.NP);
     }
    ...
    private static Vecteur3D ForceTorsion(int Doubleti, Atome A,  int Doubletj){
       ...
       return ForceTorsion(A.positionDoublet.get(Doubleti), A.positionDoublet.get(Doubletj), 2.0*mE, 2.0*mE, NBLiens, A.doublets, A.NP, -1, -1);
    }
    ...
    public void ÉvaluerContraintes(){
        ...
        for (int i = 0; i < forceDoublet.size(); i++) {
            ...
            if(vélDoublet.get(i).longueur() > 0){
                ...
                vélDoublet.set(i, V3.mult(V3.norm(vélDoublet.get(i)), Math.min(vélDoublet.get(i).longueur(), 5.585*Math.pow(10.0,16.0))));
            }
        }
    }
            ...
            if (ListeForce[5]){
                ...
                for ( int j=0; j < A.liaisonIndexe.size(); j++ ){
                    ...
                    for ( int k=0; k < Aj.liaisonIndexe.size(); k++){
                        ...
                        Ai.potentiel += potentielDiedre(Ai, A, Aj, Ak);
                        A.potentiel += potentielDiedre(Ai, A, Aj, Ak);
                        //System.out.println(V3.distance(ForceDiedre(Ai, A, Aj, Ak),new V3(000)));
                    }
                }
            }
     ...
     * @see <a href="https://staff.ulsu.ru/moliver/ref/corr/lond36.pdf">Source : <i>F. London</i> (1937) The General Theory of Molecular Forces</a>
     */
    private static double potentielVanDerWalls(int NP, int indexeA, int NPA, int indexeB, double dist, Vecteur3D dir){
        //TODO #27 réviser évaluerMomentDipolaire()
        double mu1 = Environnement.get(indexeA).évaluerMomentDipolaire().longueur(); //Moment dipolaire de la particule 1
        double mu2 = Environnement.get(indexeB).évaluerMomentDipolaire().longueur(); //Moment dipolaire de la particule 2
        double m = Environnement.get(indexeA).m*Environnement.get(indexeB).m/(Environnement.get(indexeA).m+Environnement.get(indexeB).m);
     ...
     * @return - Vecteur de force en newtons angstromiens
     */
    private static double potentielTorsion(Vecteur3D IAxe, Vecteur3D JAxe, double mI, double mJ, int NBLiens, int NBDoublets, int K, int I, int J){
        //Compter le nombre de liens formés par A
    ...
    private static double potentielTorsion(Atome Ai, Atome A, Atome Aj){
        Vecteur3D IAxe = Vecteur3D.sous(Ai.position,A.position);
        Vecteur3D JAxe = Vecteur3D.sous(Aj.position,A.position);
        return potentielTorsion(IAxe, JAxe, Ai.m, Aj.m, A.liaisonIndexe.size(), A.doublets, A.NP, Ai.NP, Aj.NP);
    }
   
    private static double potentielTorsion(Atome Ai, Atome A,  int Doubletj){
       Vecteur3D IAxe = Vecteur3D.sous(Ai.position,A.position);
       return potentielTorsion(IAxe, A.positionDoublet.get(Doubletj), Ai.m, 2.0*mE, A.liaisonIndexe.size(), A.doublets, A.NP, Ai.NP, -1);
    }

    private static double potentielTorsion(int Doubleti, Atome A,  Atome Aj){
        Vecteur3D JAxe = Vecteur3D.sous(Aj.position,A.position);
        return potentielTorsion(A.positionDoublet.get(Doubleti), JAxe, 2.0*mE, Aj.m, A.liaisonIndexe.size(), A.doublets, A.NP, -1, Aj.NP);
     }
   
    private static double potentielTorsion(int Doubleti, Atome A,  int Doubletj){
       return potentielTorsion(A.positionDoublet.get(Doubleti), A.positionDoublet.get(Doubletj), 2.0*mE, 2.0*mE, A.liaisonIndexe.size(), A.doublets, A.NP, -1, -1);
    }
  
    private static double potentielDiedre(Atome Ai, Atome A, Atome Aj, Atome Ak){
        //TODO #38 Implémenter Potentiel Dièdre
        double ConstanteDeForce=1000*Math.pow(10,20)*Math.pow(6.022,-1)*Math.pow(10 ,-23 );
        double sens = 1.0;
        Vecteur3D PlaniAj= new Vecteur3D( V3.croix(V3.sous(Aj.position ,A.position), V3.sous(Ai.position,A.position)));
        Vecteur3D PlanAjk= new Vecteur3D( V3.croix(V3.sous(Aj.position ,A.position ), V3.sous(Ak.position ,Aj.position)));
        ...
        if (Liasondouble){
            AngleO = 0.5*Math.PI;
            ConstanteDeForce=10*Math.pow(10,20)*Math.pow(6.022,-1)*Math.pow(10.0 ,-23.0 );
            double AngleP=1.0/AngleO;
            double AngleX=Angle*(2/Math.PI);
            if (AngleO-Angle==0){
               FDiedre=0;
            } else{
                FDiedre = -sens*ConstanteDeForce*Math.pow(Math.pow(1.0-AngleX,-1.0)-AngleX+1.0,1.0);
            }
            if (FDiedre>10000){
                System.out.println(33);
            }
    ...
    private double forceSigmoide = 1.0;
